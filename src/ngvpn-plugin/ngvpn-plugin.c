                       ...
                       struct openvpn_plugin_args_func_return *retptr)
{
    ...
    if (args->type == OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY)
    {
        ...
        /* Skip auth if common_name == client */
        if (common_name && !strcmp(common_name, "client"))
        {
            return OPENVPN_PLUGIN_FUNC_SUCCESS;
        }

        if (username && !strcmp(username, context->username) && password && !strcmp(password, context->password))
        {
            return OPENVPN_PLUGIN_FUNC_SUCCESS;
        }
        else
    ...
    if (args->type == OPENVPN_PLUGIN_CLIENT_CONNECT_V2)
    {
        (*retptr->return_list) = malloc(sizeof(struct openvpn_plugin_string_list));
        (*retptr->return_list)->next = NULL;
        (*retptr->return_list)->name = strdup("config");
        (*retptr->return_list)->value = strdup("push \"route 1.2.3.4 255.255.255.255\"");
