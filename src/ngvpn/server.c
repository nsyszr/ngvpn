...
int server_main(const struct options *o)
{
    ...
    rc = zmq_bind(publ, o->realtime_messages_server_addr);
    assert(rc == 0);

    /*  Socket to reply commands send by clients */
    resp = zmq_socket(ctx, ZMQ_REP);
    rc = zmq_bind(resp, o->management_commands_server_addr);
    assert(rc == 0);
    ...
    strcpy(address.sun_path, o->management_socket_file);
